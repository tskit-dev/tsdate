...
def main():
    print('tstime: Infer Times from a Inferred Tree Sequence')

    parser = argparse.ArgumentParser(
        description="Set up base data, generate inferred datasets,\
                     and process datasets.")
    parser.add_argument('--replicates', dest='reps', type=int,
    ...
    parser.add_argument('--length', dest='length', type=float, default=1e5,
                        help="length of sequence to simulate")
    parser.add_argument('--ne', dest='ne', type=float, default=1e4,
                        help="effective population size")
    parser.add_argument('--mutation_rate', dest='mut_rate', type=float,
                        default=1e-8, help="population scaled mutation rate")
    parser.add_argument('--recombination_rate', dest='rec_rate', type=float,
                        default=1e-8,
    ...
    results = {'true': list(), 'prior': list(), 'mut': list(), 'spans': list()}

    prior = make_prior(n=args.sample_size)
    for i in range(args.reps):
        print(i)
        ts = msprime.simulate(sample_size=args.sample_size,
                              length=args.length, Ne=args.ne,
        ...
        theta = 4 * args.mut_rate * args.ne
        rho = 4 * args.rec_rate * args.ne

        approx_post = get_approx_post(ts, prior_vals, time_grid,
                                      clock=args.clock, theta=theta,
                                      rho=rho, del_p=args.del_p)
        mn, var = approx_post_mean_var(ts, time_grid, approx_post)

        # Find the span of each node
        spans = list()
        for node in ts.nodes():
            if node.flags != 1:
                span = ts.tables.edges.right[ts.tables.edges.parent == node.id] - ts.tables.edges.left[ts.tables.edges.parent == node.id]
                span = np.sum(span) / args.length
                spans.append((node.id, span))

        results['true'].append(ts.tables.nodes.time[args.sample_size:])
        results['prior'].append((mixture_prior['Expected_Age'],
                                 mixture_prior['Var_Age']))
        results['mut'].append((mn[args.sample_size:], var[args.sample_size:]))
        results['spans'].append(spans)

    np.savetxt(args.output + '_true_times', np.concatenate(results['true']).ravel())
    prior_expect = np.concatenate([result[0] for result in results['prior']])
    np.savetxt(args.output + '_prior_expect', prior_expect)
    prior_var = np.concatenate([result[1] for result in results['prior']])
    np.savetxt(args.output + '_prior_var', prior_var)
    mn_results = np.concatenate([result[0] for result in results['mut']]) * 2 * args.ne
    np.savetxt(args.output + '_inferred_times', mn_results)
    var_results = np.concatenate([result[1] for result in results['mut']]) * 2 * args.ne
    np.savetxt(args.output + '_inferred_time_vars', var_results)
    np.savetxt(args.output + '_spans', [item[1] for sublist in results['spans'] for item in sublist])
