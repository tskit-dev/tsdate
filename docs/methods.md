...
demographic history. An algorithm for rescaling node ages by
on matching observed and expected segregating sites within time windows was
introduced by the ARG inference software
[SINGER](https://doi.org/10.1101/2024.03.16.585351) (Deng et al 2024). 
`tsdate` uses a modified version of this algorithm that works on gamma natural
parameters rather than point estimates, that is not impacted by the artefactual
polytomies introduced by `tsinfer`.
